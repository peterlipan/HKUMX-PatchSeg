...
def classNames = ["Soft tissue", "Tumor", "Bone", "Marrow", "Normal cartilage"]   // names of classes of interest (simply add more values to list to add more classes)
...
double percentageThreshold = 0.1  // if a patch contains less than X amount of tissue, it should be neglected (range [0.0, 1.0])
...
def pathOutput = buildFilePath(PROJECT_BASE_DIR, 'tiles_512_10x_FiveClass_IdentifyTumor')
mkdirs(pathOutput)
...
listOfFiles.each { tile ->
    ...
    def dir_subimage = new File(pathOutput + "/Images/" + tile.getName().split( )[0]);
    if (!dir_subimage.isDirectory())
        dir_subimage.mkdir()
    def dir_sublabel = new File(pathOutput + "/Labels/" + tile.getName().split( )[0]);
    if (!dir_sublabel.isDirectory())
        dir_sublabel.mkdir()
    ...
    if (amountTissue < percentageThreshold) {
        ...
        tile.renameTo(pathOutput + "/Images/"+ tile.getName().split( )[0]+ "/" + tile.getName())
        currLabelPatch.renameTo(new File(pathOutput + "/Labels/"+ tile.getName().split( )[0]+ "/" + tile.getName().split(imageExtension)[0] + ".png"))
    }
}
